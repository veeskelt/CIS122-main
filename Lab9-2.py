...
# Description: This program prompts the user for the
#   runtime of a session and calculates the total runtime of a program, with
#   options to input notes for each session, change entries and reset the
#   program's data.
...
# I changed this from calculating session times when given run times to
# calculating total run times from session times, as most applications for this
# kind of behavior supply session times and not total run-times; Steam is the
# only program I know if that doesn't track session times persistently.
...
def menu():
    """
    This function prints a menu to the user
    Thought: I should make this into a module
    :return:
    """
    option_1 = "Add a session time"
    option_2 = "Edit existing session"
    option_3 = ""
